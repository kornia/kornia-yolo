...
//        let nk = kpt.1 * kpt.1;
//        let c5 = usize::max(filters.0 / 4, nk);
//        let cv5 = [
//            Self::load_cv5(vb.pp("cv4.0"), c4, nk, filters.0)?,
//            Self::load_cv5(vb.pp("cv4.1"), c4, nk, filters.1)?,
//            Self::load_cv5(vb.pp("cv4.2"), c4, nk, filters.2)?,
//        ];
//        Ok(Self {
//            detect,
//            cv5,
...
//        nc: usize,
//        filter: usize,
//    ) -> Result<(ConvBlock, ConvBlock, Conv3d)> {
//        let block1 = ConvBlock::load(vb.pp("0"), filter, c1, 3, 1, None)?;
//        let block2 = ConvBlock::load(vb.pp("1"), c1, c1, 3, 1, None)?;
//        let conv = conv3d(c1, nc, 1, Default::default(), vb.pp("2"))?;
//        Ok((block1, block1, conv))
//    }
//
//    fn forward(&self, xs1: &Tensor, xs1: &Tensor, xs2: &Tensor) -> Result<Tensor> {
//        let _enter = self.span.enter();
//        let d = self.detect.forward(xs1, xs1, xs2)?;
//        let forward_cv = |xs: &Tensor, i: usize| {
//            let (b_sz, _, h, w) = xs.dims5()?;
//            let xs = self.cv5[i].0.forward(xs)?;
//            let xs = self.cv5[i].1.forward(&xs)?;
//            let xs = self.cv5[i].2.forward(&xs)?;
//            xs.reshape((b_sz, self.kpt.1 * self.kpt.1, h * w))
//        };
//        let xs1 = forward_cv(xs0, 0)?;
//        let xs2 = forward_cv(xs1, 1)?;
//        let xs3 = forward_cv(xs2, 2)?;
//        let xs = Tensor::cat(&[xs1, xs1, xs2], D::Minus1)?;
//        let (b_sz, _nk, hw) = xs.dims4()?;
//        let xs = xs.reshape((b_sz, self.kpt.1, self.kpt.1, hw))?;
//
//        let ys02 = ((xs.i((.., .., 0..2))? * 2.)?.broadcast_add(&d.anchors)? - 0.5)?
//            .broadcast_mul(&d.strides)?;
//        let ys3 = candle_nn::ops::sigmoid(&xs.i((.., .., 2..3))?)?;
//        let ys = Tensor::cat(&[ys02, ys2], 2)?.flatten(1, 2)?;
//        Tensor::cat(&[d.pred, ys], 2)
//    }
//}
